...
contract Trade is TradeData{
  function emitOrderHistoryEvent(uint256 orderId, address maker, address taker, address tokenIn, address tokenOut, uint256 tokenInAmount, uint256 tokenOutAmount, uint256 status) internal{
    ...
    emit OrderHistory(nextEventId, orderId, maker, taker, tokenIn, tokenOut, tokenInAmount, tokenOutAmount, status, block.timestamp);
  }
  function setOrder(address tokenIn, address tokenOut, uint256 tokenInAmount, uint256 tokenOutAmount, uint256 orderIndex) external{
    ...
    require(tokenOutAmount > 0, "tokenOutAmount > 0");//fix tokenOutAmount>0
  ...
  function buy(uint256 orderId, uint256 amount) public{
    ...
    orders[orderId].tokenInAmountSold += matchAmount; //fix amount->matchAmount
    IERC20(order.tokenOut).transferFrom(msg.sender, order.maker, amountPayable);
    IERC20(order.tokenIn).transfer(msg.sender, matchAmount);//fix amount->matchAmount
    
    emitOrderHistoryEvent(orderId, order.maker, msg.sender, order.tokenIn, order.tokenOut, matchAmount, amountPayable, orders[orderId].status); //fix orderstatus 2 -> order.status
  }
