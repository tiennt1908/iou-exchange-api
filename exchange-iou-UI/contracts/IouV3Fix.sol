...
contract IOU{
  ...
  struct TokenInfo{
    ...
    uint256 promiseTokenRate;//decimal 18 //default price 1 promiseToken ~ nCollateralToken (promiseToken/CollateralToken)
  ...
  function create(string memory name, string memory symbol,address officialToken, address collateralToken, uint256 amountCollateral, uint256 promiseTokenRate, uint256 deadline, bool isPublicPool) external{

    require(promiseTokenRate > 0 && amountCollateral > 0, "Input > 0");
    ...
    tokens[promiseToken] = TokenInfo(
      {
        ...
        totalSupplyPromiseToken: 0,
        collateralToken: collateralToken,
        totalSupplyCollateralToken: 0,
        promiseTokenRate: promiseTokenRate,
        deadline: deadline,
    ...
    //supply collateral
    supplyCollateral(promiseToken, amountCollateral);
  }
  function supplyCollateral(address tokenContract, uint256 collateralAmount) public{
    ...
    uint8 promiseTokenDecimal = IERC20(tokenInfo.promiseToken).decimals();
    uint8 collateralTokenDecimal = IERC20(tokenInfo.collateralToken).decimals();
    uint256 promiseTokenAmount = (collateralAmount * tokenInfo.promiseTokenRate * 10**promiseTokenDecimal) / 10**(18 + collateralTokenDecimal);
    ...
    eventCount = nextEventId;
    pastEvents[nextEventId] = block.number;
    emit SupplyCollateral(tokenContract, tokenInfo.collateralToken, collateralAmount);
  }
  ...
  function removeCollateral(address tokenContract, uint256 promiseTokenAmount) external{
    ...
    uint8 promiseDecimal = IERC20(tokenInfo.promiseToken).decimals();
    uint8 collateralDecimal = IERC20(tokenInfo.collateralToken).decimals();
    uint256 collateralAmountExit = (promiseTokenAmount * 10**(18 + collateralDecimal)) / (tokenInfo.promiseTokenRate * 10**promiseDecimal);
  ...
  function supplyOfficialToken(address tokenContract, uint256 officialTokenAmount) external{
    ...
    uint8 collateralDecimal = IERC20(tokenInfo.collateralToken).decimals();
    uint8 promiseDecimal = IERC20(tokenInfo.promiseToken).decimals();
    uint256 collateralAmountExit = (officialTokenAmount * 10**(18 + collateralDecimal)) / (tokenInfo.promiseTokenRate * 10**promiseDecimal);
    ...
    eventCount = nextEventId;
    pastEvents[nextEventId] = block.number;
    emit SupplyOfficialToken(tokenContract, tokenInfo.officialToken, officialTokenAmount);
  }
